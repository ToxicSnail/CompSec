...
BN BN::operator + (const BN& bNum) {
    ...
    while (j < t)
    {
        ...
        k = (BASE)(tmp >> b);// оптимизированное деление на b
        j++;
    }
    while (j < len) {
...
BN BN ::operator-(const BN& bNum) {
    ...
    while (j < bNum.len) {//c двумя коэфами
        ...
        k = !(tmp >> b);//считаем заём ( работаем с 1 из 233)
    ...
    newNum.len = len;
    while (newNum.len > 1 && newNum.coef[newNum.len - 1] == 0) {// считаем фактическую длинну
        newNum.len--;
    }
    return newNum;
...
BN BN:: operator / (const BASE& num) {
    ...
    int b = BASE_SIZE;
    while (j < len) {
        tmp = (((DBASE)r << b) + (DBASE)coef[len - 1 - j]); // r * b + coef[len-1-j]
...
BN BN::operator/(const BN& num) {
    ...
    if (newNum.len == len) {/// добавляем 1 разряд если он не добавился при умножении
        newNum.maxlen++;
        newNum.len = maxlen;
        delete[]newNum.coef;
